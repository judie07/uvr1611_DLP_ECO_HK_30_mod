...
#echo $line | sed -n -e 's/^pm/pm/p' |  sed "s/pm /null,\'$DATE\',/"| sed -r 's/\s+/,/g' | sed -re "s/,([0-9a-fA-F]+)/,unhex(\'\1\')/176g" | sed 's/.*/echo "insert ignore into t_hg_data values (&)"/e'| sed 's/,)/,null);/' 
