...
#echo $line
echo $line | sed -n -e 's/^pm/pm/p' |  sed "s/pm /null,\'$DATE\',/"| sed -r 's/\s+/,/g' | sed -re "s/,([0-9a-fA-F]+)/,unhex(\'\1\')/195g" | sed 's/.*/echo "insert ignore into t_hg_data_new values (&)"/e'| sed 's/,)/,null);/' 

echo $line | sed -n -e 's/^pm/pm/p' |  sed "s/pm /null,\'$DATE\',/"| sed -r 's/\s+/,/g' | sed -re "s/,([0-9a-fA-F]+)/,unhex(\'\1\')/195g" | sed 's/.*/echo "insert ignore into t_hg_data_new values (&)"/e'| sed 's/,)/,null);/' | mysql -u uvr1611 --password="uvr1611" --database="uvr1611"
